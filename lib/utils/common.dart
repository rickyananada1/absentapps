...
extension strEtx on String {
  Widget iconImage({double? size, Color? color, BoxFit? fit}) {
    return Image.asset(
      ...
      fit: fit ?? BoxFit.cover,
      color: color ??
          (appStore.isDarkModeOn.value ? Colors.white : gray.withOpacity(0.6)),
    ...
    children: [
      Container(
        ...
        decoration: boxDecorationWithRoundedCorners(
            boxShape: BoxShape.circle, backgroundColor: appColorPrimary),
...
Widget settingIconWidget({IconData? icon}) {
  return Container(
    ...
    decoration:
        const BoxDecoration(color: appColorPrimary, shape: BoxShape.circle),
...
Future<void> commonLaunchUrl(String address,
  ...
  if (await launchUrl(Uri.parse(address), mode: launchMode)) {
